...
/***基本変数***/
 var ccbs = {
           ...
           /*functions*/
             'string':{
                'list':[['math','数学'],['image','画像'],['encode','エンコード'],['decode','デコード'],['url','url'],['text',"テキスト"],['recipe','レシピ'],['weather','気象情報']]
               ...
               ,'root':{
                       'list':['wolf','ewolf','sage','検索','画像検索','Twitter検索','ヤフー検索','マップ検索','乗り換え検索','YouTube検索','ウィキ検索',"経路検索",'翻訳','英訳','レート',"実行","ニュース"]
                      ,'devlist':['wolf','ewolf','sage','google','image','twitter','yahoo','map','norikae','youtube','wiki',"route",'translate','toEng','rate',"runCode","news"]
                      ,'wolf':f_wolf
               ...
               ,'math':{
                ...
                ,'image':{
                         'list':[['paint','着色'],['grayscale','白黒'],['resize','リサイズ'],['crop','切り出し'],['radius','丸く'],['border','枠']],
                         'paint':f_image_paint,
                         'grayscale':f_image_grayscale,
                         'resize':f_image_resize,
                         'crop':f_image_crop,
                         'radius':f_image_radius,
                         'border':f_image_border
                }
            ...
            /*datas*/
                'htu':{
                 /*how to use*/
                      ...
                      ,'sage':'sage〇〇\nと送信すると、〇〇をSAGE Mathで実行した出力を返します。'
                      ...
                      /*数学*/
                       ...
                       ,'image':{
                               'list':[['paint','着色'],['grayscale','白黒'],['resize','リサイズ'],['crop','切り出し'],['radius','丸く'],['border','枠']],
                               'paint':"白黒写真などをAIで着色します",
                               'grayscale':"写真を白黒写真にします。",
                               'resize':"横(px|倍率|w|h),縦(px|倍率|w|h),画像URLの順に指定すると、画像を指定したサイズに引き伸ばします。",
                               'crop':"横(px|倍率|w|h),縦(px|倍率|w|h),基準(省略可),URLの順に指定すると、指定したサイズに画像を切り取ります。"
                                    +"\n基準は、上下左右または「顔」が指定できます。",
                               'radius':"画像を丸く切り取ります。",
                               'border':"太さ(px),色(英語で。rgbを使う場合は、rgb:ffffffの様に指定してください。),URLの順に指定すると、写真に枠を追加します。"
                       }
                       ,'encode':{
                               'list':[['binary','バイナリ'],['utf8','utf8'],['utf16','utf16'],['utf32','utf32'],['escape','escape'],['url','url'],['qurl','qurl'],['base64','base64'],['html','html']]
 ...
 *******************/
 function f_ccbs_s(mes){
  ...
  if(mes.slice(0,4).toLowerCase()=='sage'){
   var key=mes.slice(4);
   if(key){
    return curobj.root.sage(key);
   }else{
    return curhtuobj.sage;
   }
  }else
  if(mes.slice(0,2)=="検索"){
...
function f_image_paint(url){
  try{
    var request=UrlFetchApp.fetch("https://api.deepai.org/api/colorizer",{
      method:'post',
      headers:{
        'api-key':"a62dd000-c984-4430-aab1-8ee907b7c810",
       'Content-Type':"application/x-www-form-urlencoded"
      },
      payload:"image="+encodeURIComponent(url)
    }).getContentText();
    try{
      return [{type:'image',url:JSON.parse(request).output_url}]
    }catch(e){
      return request;//エラーテキスト?
    }
  }catch(e){
    return "エラーが発生しました。";
  }
...
function f_image_grayscale(url){
  var nurl="https://res.cloudinary.com/demo/image/fetch/e_grayscale/"+encodeURIComponent(url);
  try{
    ajax_GET(nurl);
    return [{type:'image',url:nurl}];
  }catch(e){
    return "白黒化できませんでした";
  }
}
...
function f_image_resize(text){
  ...
  if(ts.length==3){
    ...
    return "値は,または改行区切りで横(px),縦(px),URLの順に指定してください。";
  }
  var nurl="https://res.cloudinary.com/demo/image/fetch/w_"+width+",h_"+height+",c_scale/"+encodeURIComponent(url);
  try{
    ajax_GET(nurl);
    return [{type:'image',url:nurl}];
  }catch(e){
...
function f_image_crop(text){
  ...
  if(ts.length>=3){
    ...
    url=ts[2];
    if(ts.length==4){
      gravity=",g_"+ts[2].replace(/上/g,'north').replace(/下/g,'south').replace(/左/g,'west').replace(/右/g,'east').replace(/顔/g,'face');
      url=ts[3];
    }
  }else{
    return "値は,または改行区切りで横(px),縦(px),基準(省略可),URLの順に指定してください。"
          +"\n基準は、上下左右または「顔」が指定できます。";
  }
  var nurl="https://res.cloudinary.com/demo/image/fetch/w_"+width+",h_"+height+gravity+",c_crop/"+encodeURIComponent(url);
  try{
    ajax_GET(nurl);
    return [{type:'image',url:nurl}];
  }catch(e){
...
function f_image_radius(url){
  var nurl="https://res.cloudinary.com/demo/image/fetch/r_max/"+encodeURIComponent(url);
  try{
    ajax_GET(nurl);
    return [{type:'image',url:nurl}];
  }catch(e){
...
function f_image_border(text){
  ...
  if(ts.length==1){
    ...
    return "値は,または改行区切りで,太さ(px),色(英語で。rgbを使う場合は、rgb:ffffffの様に指定してください。),URLの順に指定してください。"
  }
  var nurl="https://res.cloudinary.com/demo/image/fetch/bo_"+weight+"px_solid_"+color+"/"+encodeURIComponent(url);
  try{
    ajax_GET(nurl);
    return [{type:'image',url:nurl}];
  }catch(e){
